...
// delete user appointment
  ...
  Future<ApiResponse> getrepnotificationList({
    ...
    try {
      Response response =
          await dioClient1.get(ApiConstant.getAllAnnouncements, data: {
        'executive_id': userId,
      });
      return ApiResponse.withSuccess(response);
    } catch (e) {
...
// leader notification
  ...
  getstudentnotificationList({required String userId}) async {
    try {
    Response response =
    await dioClient1.get(ApiConstant.getStudentCourses, data: {
    'user_id': userId,
    });
    return ApiResponse.withSuccess(response);
    } catch (e) {
